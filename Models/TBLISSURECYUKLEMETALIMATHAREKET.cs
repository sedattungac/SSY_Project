//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace SSY_Project.Models
{
    using System;
    using System.Collections.Generic;
    
    public partial class TBLISSURECYUKLEMETALIMATHAREKET
    {
        public int IND { get; set; }
        public Nullable<int> STOKIND { get; set; }
        public Nullable<int> EVRAKNO { get; set; }
        public Nullable<int> TALIMATIND { get; set; }
        public Nullable<int> MLZTESLIMIND { get; set; }
        public string ACIKLAMA { get; set; }
        public Nullable<decimal> MIKTAR { get; set; }
        public Nullable<decimal> KESIMOLCUSU { get; set; }
        public Nullable<decimal> PLAKAADET { get; set; }
    }
}
